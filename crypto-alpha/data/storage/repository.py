...
class CandleRepository:
    """SQLite repository for OHLCV candle data."""
    ...
    def get_range(self, symbol: str, start_ts: int, end_ts: int) -> list[Candle]:
        """Get candles in time range.

        Args:
            symbol: Trading pair symbol
            start_ts: Start timestamp (inclusive) in milliseconds
            end_ts: End timestamp (inclusive) in milliseconds
    ...
    def count(self, symbol: str | None = None) -> int:
        ...
        Args:
            symbol: Optional trading pair symbol. If None, counts all candles.
